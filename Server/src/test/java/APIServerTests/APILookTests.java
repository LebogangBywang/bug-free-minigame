...
public class APILookTests {
    ...
    public void validLookShouldSucceed() throws UnirestException {
        ...
        response = Unirest.post("http://localhost:" + port + "/robot/hal")
                .header("Content-Type", "application/json")
                .body("{\"robot\": \"hal\", \"command\": \"look\", \"arguments\": []}")
                .asJson();
        assertEquals(200, response.getStatus());
        assertEquals("OK", response.getBody().getObject().get("result").toString());
        assertNotNull(response.getBody().getObject().get("result"));
        assertNotNull(response.getBody().getObject().get("state"));
        assertNotNull(((JSONObject)response.getBody().getObject().get("state")).get("position"));
